...
do_KLdivergence_post2pr=0 # check effect KL divergence (prior as reference curve; parametric, based on computational model).
do_perceived_certaintyE1_confidence_KLdivergence=0 # check effect perceived own certainty and perceived peer confidence, as well as KL divergence (all parametric, based on computational model) 
do_stayBias=0 # check effect stay bias, modulated by peer confidence level (parametric)
...
dir_fsf_certaintyE1_confidence='/home/lhofman/Projects/mushroom_tux/analysis/first_level/scripts/certaintyE1_confidence.fsf' 
fsf_certaintyE1_confidence='certaintyE1_confidence.fsf'
dir_fsf_perceived_certaintyE1_confidence='/home/lhofman/Projects/mushroom_tux/analysis/first_level/scripts/perceived_certaintyE1_confidence.fsf' 
fsf_perceived_certaintyE1_confidence='perceived_certaintyE1_confidence.fsf'
dir_fsf_KLdivergence_post2pr='/home/lhofman/Projects/mushroom_tux/analysis/first_level/scripts/KLdivergence_post2pr.fsf' 
fsf_KLdivergence_post2pr='KLdivergence_post2pr.fsf'
dir_fsf_perceived_certaintyE1_confidence_KLdivergence='/home/lhofman/Projects/mushroom_tux/analysis/first_level/scripts/perceived_certaintyE1_confidence_KLdivergence.fsf' 
fsf_perceived_certaintyE1_confidence_KLdivergence='perceived_certaintyE1_confidence_KLdivergence.fsf'
dir_fsf_stayBias='/home/lhofman/Projects/mushroom_tux/analysis/first_level/scripts/stayBias.fsf' 
...
if [[ $do_mountFMG == 1 ]]; then 
    # old drive
    #mkdir -p /home/lhofman/fmgmount/mushroom
    #sudo mount -t cifs "//fmgstorage.fmg.uva.nl/psychology$/fMRI Projects/fMRI Project Mushroom" /home/lhofman/fmgmount/mushroom -o username=lhofman,domain=uva,noexec,uid=$(id -u),gid=$(id -g)
    # new drive
        ...
        if printf '%s\n' ${exclusions[@]} | grep -q --line-regexp $subnr; then
            ...
            for run in $subdir/func/*_bold.nii.gz; do  
                ...
                if [[ $do_certaintyE1_confidence == 1 ]]; then 
                    ...
                    EV1=$subdir/func/$sub'_task-mushroom_run-'$run_number'_condition-lowCertaintyE1_events.txt'
                    EV2=$subdir/func/$sub'_task-mushroom_run-'$run_number'_condition-highCertaintyE1_events.txt'
                    EV3=$subdir/func/$sub'_task-mushroom_run-'$run_number'_condition-parametricConfidenceSI_events.txt'
                ...
                if [[ $do_perceived_certaintyE1_confidence == 1 ]]; then 
                    ...
                    funcfile=$dir_bids/derivatives/halfpipe/$sub/func/$sub'_task-mushroom_run-'$run_number'_setting-preproc_bold'
                    conffile=$dir_bids/derivatives/halfpipe/$sub/func/$sub'_task-mushroom_run-'$run_number'_setting-preproc_desc-confounds_regressors_selection_no_ICAAROMA.tsv'
                    EV1=$subdir/func/$sub'_task-mushroom_run-'$run_number'_params-perceivedCertainty_events.txt'
                    EV2=$subdir/func/$sub'_task-mushroom_run-'$run_number'_params-perceivedConfidence_events.txt'
                    NPT=$(grep 'NumberOfVolumes' $funcfile.json | sed -r 's/^[^:]*:\ (.*),$/\1/') # using jq would be more optimal, but I can't download it.     
                ...
                # KL divergence (prior/E1 as reference) (= belief update)
                # ============================================================== 
                if [[ $do_KLdivergence_post2pr == 1 ]]; then 

                    # 1. Copy the design file into the subject directory
                    outputdir=$dir_bids/derivatives/first_level/$sub/KLdivergence_post2pr/run-$run_number'_output'
                    mkdir -p $outputdir
                    ...
                    funcfile=$dir_bids/derivatives/halfpipe/$sub/func/$sub'_task-mushroom_run-'$run_number'_setting-preproc_bold'
                    conffile=$dir_bids/derivatives/halfpipe/$sub/func/$sub'_task-mushroom_run-'$run_number'_setting-preproc_desc-confounds_regressors_selection_no_ICAAROMA.tsv'
                    EV1=$subdir/func/$sub'_task-mushroom_run-'$run_number'_params-KLdpost2pr_events.txt'
                    NPT=$(grep 'NumberOfVolumes' $funcfile.json | sed -r 's/^[^:]*:\ (.*),$/\1/') # using jq would be more optimal, but I can't download it.     
                ...
                # Perceived certainty at E1 and perceived confidence at E2, as well as KL divergence
                # ============================================================== 
                if [[ $do_perceived_certaintyE1_confidence_KLdivergence == 1 ]]; then 

                    # 1. Copy the design file into the subject directory
                    outputdir=$dir_bids/derivatives/first_level/$sub/perceived_certaintyE1_confidence_KLdivergence/run-$run_number'_output'
                    mkdir -p $outputdir
                    ...
                    EV1=$subdir/func/$sub'_task-mushroom_run-'$run_number'_params-perceivedCertainty_events.txt'
                    EV2=$subdir/func/$sub'_task-mushroom_run-'$run_number'_params-perceivedConfidence_events.txt'
                    EV3=$subdir/func/$sub'_task-mushroom_run-'$run_number'_params-KLdpost2pr_events.txt'
                    NPT=$(grep 'NumberOfVolumes' $funcfile.json | sed -r 's/^[^:]*:\ (.*),$/\1/') # using jq would be more optimal, but I can't download it.     
                ...
                if [[ $do_stayBias == 1 ]]; then 
                    ...
                    # 1. Copy the design file into the subject directory
                    outputdir=$dir_bids/derivatives/first_level/$sub/stayBias/run-$run_number'_output'
                    mkdir -p $outputdir
                    cp $dir_fsf_stayBias $outputdir
                
                    # 2 Adapt subject-specific information in the design file for stay bias      
                    funcfile=$dir_bids/derivatives/halfpipe/$sub/func/$sub'_task-mushroom_run-'$run_number'_setting-preproc_bold'
                    conffile=$dir_bids/derivatives/halfpipe/$sub/func/$sub'_task-mushroom_run-'$run_number'_setting-preproc_desc-confounds_regressors_selection_no_ICAAROMA.tsv'
                    EV1=$subdir/func/$sub'_task-mushroom_run-'$run_number'_params-stayBias_events.txt'
                    NPT=$(grep 'NumberOfVolumes' $funcfile.json | sed -r 's/^[^:]*:\ (.*),$/\1/') # using jq would be more optimal, but I can't download it.     
...
if [[ $do_QA == 1 ]]; then 
    ...
    touch $dir_bids/derivatives/first_level/QA/lev1_QA_KLdivergence_post2pr.html
    touch $dir_bids/derivatives/first_level/QA/lev1_QA_perceived_certaintyE1_confidence_KLdivergence.html
    touch $dir_bids/derivatives/first_level/QA/lev1_QA_stayBias.html
