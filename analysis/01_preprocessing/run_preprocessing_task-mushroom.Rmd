...
# directories
project_wd <- "P:/fMRI Projects/fMRI Project Mushroom"
setwd(project_wd)
...
for (iSubject in list_subjects){
  mushroomfiles <- list.files(path = file.path(iSubject, "beh"), pattern = "sub-.*_task-mushroom_run-prac.*beh.*.csv", full.names = TRUE)
...
pracdata <- df_prac[!df_prac$sID %in% participants_to_exclude$exclude_beh,]
...
mushroomdata <- df_all[!df_all$sID %in% participants_to_exclude$exclude_beh,]

# remove incomplete datasets
trials_pp <- mushroomdata %>% 
  dplyr::count(sID) %>%
  filter(n == 75)
mushroomdata <- mushroomdata[mushroomdata$sID %in% trials_pp$sID,] # - 2 participants = 71pp
...
# Add participants’ updating strategies: 0, 0<s>1, 1
...
to_exclude <- c(122) # qualitative check by Lieke Hofmans
data_clean <- mushroomdata[!mushroomdata$sID %in% to_exclude,] # -1 participants  = 70 pp with 5250 trials

# ii. filler trials
data_clean <- data_clean[data_clean$filler_trial==0,] # 5250 - 1050 = 4200 trials
...
data_clean <- data_clean[!is.na(data_clean$estimate2_rt),] # -1; 4199 trials
a <- data_clean[data_clean$estimate1_rt > 20 | data_clean$estimate2_rt > 20,]
data_clean <- data_clean[data_clean$estimate1_rt <= 20 & data_clean$estimate2_rt <= 20,] # 4199 - 24 = 4175 trials

# iv. trials with either estimate 1 or estimate 2 RT < 0.2 sec.
data_clean <- data_clean[data_clean$estimate1_rt >= 0.2 & data_clean$estimate2_rt >= 0.2,] # 4175 - 0 = 4175 trials
...
data_clean <- data_clean[abs(data_clean$estimate1 - data_clean$estimate_peer) > 12 & abs(data_clean$estimate1 - data_clean$estimate_peer) < 19,] # 4175 - 2 = 4173 trials
...
data_clean <- data_clean[data_clean$trial_filter==1,] # 4173 - 75 = 4098 trials
...
data_clean <- data_clean[data_clean$trial_filter==1,] # 4098 - 14 = 4084 trials
...
data_clean <- data_clean[!data_clean$sID %in% to_exclude,] # -2 participants = 3919 trials left (pp 109, 110)

# how many participants left?
length(unique(data_clean$sID)) # 68
...
# viii. Exclude participants with S = 0 (stay) on more than 70% of trials
freq_strategies <- data_clean_allTrials%>%
  select(sID, strategy)%>%
  group_by(sID, strategy)%>%
  dplyr::summarise(n = n())%>%
  dplyr::mutate(strategy_freq=n/sum(n))
to_exclude <- freq_strategies$sID[freq_strategies$strategy=="stay" & freq_strategies$strategy_freq>0.70] # -2 participants (pp 109, 110)
data_clean_allTrials <- data_clean_allTrials[!data_clean_allTrials$sID %in% to_exclude,] 

# how many participants left?
length(unique(data_clean_allTrials$sID)) # 68
...
data_clean_include_anticonformity <- data_clean_include_anticonformity[!is.na(data_clean_include_anticonformity$estimate2_rt),] 
data_clean_include_anticonformity <- data_clean_include_anticonformity[data_clean_include_anticonformity$estimate1_rt <= 20 & data_clean_include_anticonformity$estimate2_rt <= 20,] 

# iv. trials with either estimate 1 or estimate 2 RT < 0.2 sec.
data_clean_include_anticonformity <- data_clean_include_anticonformity[data_clean_include_anticonformity$estimate1_rt >= 0.2 & data_clean_include_anticonformity$estimate2_rt >= 0.2,] 

# v. trials without suitable social info (suitable = 15-18 mushrooms from estimate 1, but could be slightly lower (13-14) in case E1 was very close to 0  or 1)
data_clean_include_anticonformity <- data_clean_include_anticonformity[abs(data_clean_include_anticonformity$estimate1 - data_clean_include_anticonformity$estimate_peer) > 12 & abs(data_clean_include_anticonformity$estimate1 - data_clean_include_anticonformity$estimate_peer) < 19,] # 
...
# viii. Exclude participants with S = 0 (stay) on more than 70% of trials
freq_strategies <- data_clean_include_anticonformity%>%
  select(sID, strategy)%>%
  group_by(sID, strategy)%>%
  dplyr::summarise(n = n())%>%
  dplyr::mutate(strategy_freq=n/sum(n))
...
ggplot(data = summ, aes(x = as.factor(nTotal), y = confidence_rating)) +
  ...
  theme_classic(base_size = 16)
t.test(x = prac_summ$confidence_rating[prac_summ$nTotal==5], y = prac_summ$confidence_rating[prac_summ$nTotal==45], paired = TRUE) # nTotal = 5: 1.61, nTotal = 45: 2.28; t = -7.54, df = 70, p < 0.001
...
participants_to_exclude$exclude_mushroom[nrow(participants_to_exclude)] <- 122
participants_to_exclude$reason[nrow(participants_to_exclude)] <- "did not adhere to task instructions"
participants_to_exclude[nrow(participants_to_exclude)+1, ] <- NA
participants_to_exclude$exclude_mushroom[nrow(participants_to_exclude)] <- 109
participants_to_exclude$reason[nrow(participants_to_exclude)] <- ">70% stay trials"
participants_to_exclude[nrow(participants_to_exclude)+1, ] <- NA
participants_to_exclude$exclude_mushroom[nrow(participants_to_exclude)] <- 110
participants_to_exclude$reason[nrow(participants_to_exclude)] <- ">70% stay trials"
