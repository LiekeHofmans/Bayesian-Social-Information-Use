...
# Set directories
DIR_analysis_main_1 <- "P:/fMRI Projects/fMRI Project Mushroom/0_online_experiment/analysis"
DIR_analysis_1 <- "P:/fMRI Projects/fMRI Project Mushroom/0_online_experiment/analysis/comp_model"
DIR_data_1 <- "P:/fMRI Projects/fMRI Project Mushroom/0_online_experiment/data"
DIR_analysis_main_2 <- "P:/fMRI Projects/fMRI Project Mushroom/analysis"
DIR_analysis_2 <- "P:/fMRI Projects/fMRI Project Mushroom/analysis/04_level-group/mushroom"
DIR_data_2 <- "P:/fMRI Projects/fMRI Project Mushroom/bids/derivatives/beh/mushroom"
fit.folder <- 'results_fit_sample'
prediction.folder <- 'results_pred' 
simulation.folder <- 'results_sim'

source(file.path(DIR_analysis_main_2, "summary_function.R"))

# Load data
df_mushroom_1 <- read.csv(file.path(DIR_data_1, "4. behdata_wave2_mushroomTask_confidence_preproc.csv"))
df_mushroom_1 <- df_mushroom_1 %>% select(sID, trialNumber, nTotal, true_ratio, nMushrooms, nBlue, nRed, estimate1, estimate2, estimate_peer, confidence_peer, s)
colnames(df_mushroom_1)[colnames(df_mushroom_1) == "trialNumber"] <- "trial_number"
df_mushroom_1$sID <- df_mushroom_1$sID + 1000
df_mushroom_2 <- read.csv(file.path(DIR_data_2, "group_task-mushroom_preproc-trialwise.csv"))
df_mushroom_2 <- df_mushroom_2 %>% select(sID, trial_number, nTotal, true_ratio, nMushrooms, nBlue, nRed, estimate1, estimate2, estimate_peer, confidence_peer, s)

d <- rbind(df_mushroom_2, df_mushroom_1)

# Load models
source(file.path(DIR_analysis_2, "model_specification.R"))

# Load helper functions
source(file.path(DIR_analysis_2, "helper_functions.R"))
...
for(m in ms[c(2:10)]){
  param_values <- rbind(
    read.table(file.path(DIR_analysis_1, fit.folder, paste0('subject_', m, '_fitE2.txt')), sep = " ", header=T),
    read.table(file.path(DIR_analysis_2, fit.folder, paste0('subject_', m, '_fitE2.txt')), sep = " ", header=T)
  )
...
for(m in ms[c(2:10)]){
  ...
  write.table(results, file.path(DIR_analysis_2,simulation.folder,paste0('sim_generatedFrom',m,'.txt')))
}
...
process_model_data <- function(model_name, d, prob_stay_model = FALSE) {
  ...
  simulated_data <- read.table(file.path(DIR_analysis_2, simulation.folder, paste0('sim_generatedFrom', model_name, '.txt')), sep = " ", header = TRUE)
...
for (gen_model in ms[2:10]) {
  ...
  for (m in ms[2:10]) {
    ...
    # Output file
    output_file <- file.path(DIR_analysis_2, simulation.folder,
...
param_values <- as.data.frame(read.table(file.path(DIR_analysis_2, simulation.folder, 'combinedExperiments_fit_M3d_dataGeneratedFromM3d.txt'), sep = " ", header=T))
...
for (i_model in model_names) {
  ...
  param_values <- as.data.frame(read.table(file.path(DIR_analysis_2, simulation.folder, paste0('combinedExperiments_fit_', i_model,'_dataGeneratedFrom', i_model, '.txt')), sep = " ", header=T))
...
params_1 <- as.data.frame(read.table(file.path(DIR_analysis_1, fit.folder, paste0('subject_M3d_fitE2.txt')), sep = " ", header=T))
params_2 <- as.data.frame(read.table(file.path(DIR_analysis_2, fit.folder, paste0('subject_M3d_fitE2.txt')), sep = " ", header=T))
estimated_params <- rbind(params_2, params_1)
