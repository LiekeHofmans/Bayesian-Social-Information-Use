...
# Set directories
DIR_analysis_main <- "P:/fMRI Projects/fMRI Project Mushroom/analysis/"
DIR_this_analysis <- "P:/fMRI Projects/fMRI Project Mushroom/analysis/04_level-group/mushroom"
DIR_data <- "P:/fMRI Projects/fMRI Project Mushroom/bids/derivatives/beh/mushroom"
...
for(m in ms[c(1:10)]){ # model M0 has no parameters to fit
  ...
  print(m)
  sfStop()
  write.table(r,file.path(DIR_this_analysis,fit.folder,paste0('subject_',m,'_fitE2.txt'))) # best fit for all parameters, for each participant separately. 
...
# which models win for how many pp? (
...
# ------------------------------------------------------------------------------
# 5 Calculate model-based certainty 
# ------------------------------------------------------------------------------

# Load data
all_params <- as.data.frame(read.table(file.path(DIR_this_analysis, fit.folder, paste0('subject_M3d_fitE2.txt')), sep = " ", header=T))

# Add model based certainty variables
...
all_params$subj_low <- all_params$V4
all_params$subj_medium <- all_params$V4 + all_params$V5
all_params$subj_high <- all_params$V4 + all_params$V5 * 2
...
apply(all_params[, c(10, 11, 12)], 2, median, na.rm = TRUE) # median model based certainty per peer confidence level
apply(all_params[, c(8, 9)], 2, median, na.rm = TRUE) # median model based certainty per condition
wilcox.test((all_params[,8]), (all_params[,9]), paired = TRUE) # difference in model based certainty between uncertain and certain condition

# Graph
all_params_long <- all_params %>%
  pivot_longer(cols = c(subj_5, subj_45, subj_low, subj_medium, subj_high), 
               names_to = "variable", 
               values_to = "value")

all_params_long$variable <- factor(all_params_long$variable, 
                                   levels = c("subj_5", "subj_45", "subj_low", "subj_medium", "subj_high"),  # Manually set the order
                                   labels = c("uncertain", "certain", "low peer\nconfidence", "medium peer\nconfidence", "high peer\nconfidence"))  # Manually rename


all_params_long$group <- ifelse(all_params_long$variable %in% c("uncertain", "certain"), "green", "blue")

ggplot(all_params_long, aes(x = variable, y = value, fill = group)) +
  geom_boxplot(outlier.shape = NA) +
  scale_fill_manual(values = c("green" = "#29AF7FFF", "blue" = "#39568CFF")) + # Custom fill colors
  geom_vline(xintercept = 2.5, linetype = "dashed", color = "black", size = 1) +
  theme_classic(base_size = 36) +
  coord_cartesian(ylim = c(0, 750)) + 
  labs(x = "", y = "model-based certainty") +
...
#y_limits <- c(0.020, 0.18, 0.0087, 0.007, 2.4) # best scaling for adults + adolescents
y_limits <- c(0.025, 0.15, 0.0087, 0.0135, 2.4) # best scaling for adult exp1 + exp 2
...
for(i_model in c(10)){
  ...
  grid.arrange(grobs = list_plots, ncol = (ncol(param_values)-2))
} 
