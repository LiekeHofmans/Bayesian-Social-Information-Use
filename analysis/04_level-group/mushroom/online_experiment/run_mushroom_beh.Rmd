---
title: "4. wave2_mushroomTask_confidence"
...
analysis_wd <- getwd()
derivatives_wd <- "P:/fMRI Projects/fMRI Project Mushroom/0_online_experiment/data"

source("summary_function.R")

# load mushroom data
df_mushroom <- read.csv(file.path(derivatives_wd, "4. behdata_wave2_mushroomTask_confidence_preproc.csv"))
...
# retain only 1 row per participant
df_participants <- df_mushroom_clean %>%
  group_by(sID) %>%
  slice(1) %>%
  ungroup()
...
nrow(df_participants[df_participants$sex=="Female",])

# Age
ggplot(data = df_participants, aes(x = age)) +
  geom_histogram(binwidth = 1, color = "black", fill = "grey") + 
  theme_classic(base_size = 30) +
  ggtitle("adults")
range(df_participants$age, na.rm=T); mean(df_participants$age, na.rm=T); sd(df_participants$age, na.rm=T)
...
# own certainty 45 vs 5: beta = -0.05, p < 0.001
# peer confidence: beta = 0.09, p < 0.001
# own certainty x peer confidence: beta = -0.02, p = 0.0124
