...
#  5 Posterior predictives            - Calculate predicted data based on fitted parameters
#  6 Plot parameter distribution      - Plot the distribution of all parameters of winning model
#  7 Plot model vs behavior           - Plot predicted behavior vs observed behavior
...
# Set directories
DIR_analysis_main <- "C:/Users/lieke.hofmans/OneDrive/UvA/projects/beh_SocialInformation_Uncertainty_and_Proximity_adults/analysis"
DIR_this_analysis <- "C:/Users/lieke.hofmans/OneDrive/UvA/projects/beh_SocialInformation_Uncertainty_and_Proximity_adults/analysis/comp_model"
DIR_data <- "C:/Users/lieke.hofmans/OneDrive/UvA/projects/beh_SocialInformation_Uncertainty_and_Proximity_adults/data"

# Create folder in which the results will be stored
fit.folder <- 'results_fit_sample_correct' ; dir.create(file.path(DIR_this_analysis,fit.folder),showWarnings = T)
# Create folder in which the predicted results will be stored
prediction.folder <- 'results_pred_correct' ; dir.create(file.path(DIR_this_analysis,prediction.folder),showWarnings = T)
# Create folder in which the simulated results will be stored
simulation.folder <- 'results_sim_correct' ; dir.create(file.path(DIR_this_analysis,simulation.folder),showWarnings = T)
...
# load the data
d <- read.csv(file.path(DIR_data, '4. behdata_wave2_mushroomTask_confidence_ranef.csv'))
...
#lowest_BIC <- ms[c(5,9)][lowest_BIC] # only when comparing selected models
...
## =============================================================================
## 6 Plot parameter distribution
## =============================================================================

param_names <- c("alpha", "alpha_5", "alpha_45", "theta", "theta_IC", "theta_slope", "theta_low", "theta_med", "theta_high", "stay_bias")
