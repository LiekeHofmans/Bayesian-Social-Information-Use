...
Includes: 
- A trial-level dataframe (group_task-IST_preproc_trialwise.csv), which contains all primary variables for all individual trials + effort condition. 
- A condition x subject level dataframe (group_task-IST_preproc_conditionwise.csv), which is long-wise with 3 values for each subject for each effort condition (low, medium, high).
- A subject-level dataframe (group_task-IST_preproc_participantwise.csv), which contains the effort condition-level means for all primary variables for each participant. The effort level is denoted at the end of each variable (e.g., "boxesFlipped_low", "boxesFlipped_medium", etc.).
