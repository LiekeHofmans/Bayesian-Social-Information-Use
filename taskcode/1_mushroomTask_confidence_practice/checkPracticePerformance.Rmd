...
datafile <- read.csv(selected_file, check.names = FALSE)
# if for some reason it doesnt pick te right file, manually fill it in here below, inside the double quotes: 
# datafile <- read.csv("")
colnames(datafile)[1] <- gsub('^...', '', colnames(datafile)[1]) # to remove byte order mark from beginning of first column name
